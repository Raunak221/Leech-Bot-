...
def message_filter_f(flt, c: Client, m: Message):
    return bool(
        (
            ...
            m.document
            and m.document.file_name.upper().endswith(".TORRENT")
        )
        or
        # below checks if it is a valid link
        (
            (("http" in m.text) or ("magnet:" in m.text))
            and (
...
message_fliter = filters.create(func=message_filter_f, name="TstMesgFilter")
